...
$sql_code = "select contador,Local, Técnico, DataHora,Status,servico from chamados1 ORDER BY contador DESC LIMIT $item, $itens_por_pagina";
$execute = $conn->query($sql_code) or die($conn->error);
$produto = $execute->fetch_assoc();
$num = $execute->num_rows;
$num_total = $conn->query("select contador,Local, Técnico, DataHora,Status,servico from chamados1")->num_rows;
...
<div class="container">
	...
	<nav>
		<ul class="pagination">
			<li>
				<a href="Ver-chamados-admin.php?pagina=0" aria-label="Previous">
			...
			<?php 
				for($i=0;$i<$num_paginas;$i++){
				    ...
				    if($pagina == $i)
				    	$estilo = "class=\"active\"";
			?>
			<li <?php echo $estilo; ?> ><a href="Ver-chamados-sadmin.php?pagina=<?php echo $i; ?>"><?php echo $i+1; ?></a></li>
				<?php } ?>
				<li>
					<a href="Ver-chamados-sadmin.php?pagina=<?php echo $num_paginas-1; ?>" aria-label="Next">
