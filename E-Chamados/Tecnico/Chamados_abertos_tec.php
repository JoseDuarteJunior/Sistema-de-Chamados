...
$sql_code = "select contador,Local, Técnico, DataHora,Status,servico from chamados1 WHERE Status='Aberto' AND Técnico='$tecnico'  ORDER BY contador DESC LIMIT $item, $itens_por_pagina";
$execute = $conn->query($sql_code) or die($conn->error);
$produto = $execute->fetch_assoc();
$num = $execute->num_rows;
$num_total = $conn->query("select contador,Local, Técnico, DataHora,Status,servico from chamados1 WHERE Status='Aberto' AND Técnico='$tecnico'")->num_rows;
				 ...
				 <thead>
					...
					<tbody>
						...
						<tr>
							...
							<?php } ?>
							<td> <a class="btn btn-info btn-sm" href="fazer-chamado-tec.php?chamado=<?php echo $produto['contador'];?>" data-toggle="tooltip" title="Fazer Chamado"><span class="glyphicon glyphicon-edit"></span>Fazer</button></td>
						    </tr>
  ...
  <nav>
				  <ul class="pagination">
				    ...
				    if($pagina == $i)
				    	$estilo = "class=\"active\"";
				    ?>
				    <li <?php echo $estilo; ?> ><a href="Chamados_abertos_tec.php?pagina=<?php echo $i; ?>"><?php echo $i+1; ?></a></li>
					<?php } ?>
				    <li>
